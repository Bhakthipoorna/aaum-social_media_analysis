def TwitterAnalysis(uri,mydb):
	...
	def fetch_and_transform(uri,mydb):
		...
		#df_dumped=df_dumped[df_dumped['createdAt']>'2022-08-13']
		#df_dumped=df_dumped[df_dumped['createdAt']<'2022-08-20']
		...
		return df_tweet_count, df_daily_tweetdist, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity,df_positive_profile, df_negative_profile

	def pushdata(df_tweet_count, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity):

		def trucation(df_tweet_count, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity):

			conn = psycopg2.connect(database="psone_flash_base", user='psone', password='psone', host='localhost')  
			cursor = conn.cursor()
			delete1="DELETE FROM tw_tweetcount"
			cursor.execute(delete1)
			conn.commit()
			delete2="DELETE FROM tw_tweetdist"
			cursor.execute(delete2)
			conn.commit()
			delete3="DELETE FROM tw_freq_words"
			cursor.execute(delete3)
			conn.commit()
			delete4="DELETE FROM tw_nps"
			cursor.execute(delete4)
			conn.commit()
			delete5="DELETE FROM tw_reach"
			cursor.execute(delete5)
			conn.commit()
			delete6="DELETE FROM tw_popular"
			cursor.execute(delete6)
			conn.commit()
			delete7="DELETE FROM tw_more_tweets"
			cursor.execute(delete7)
			conn.commit()
			delete8="DELETE FROM tw_entity"
		...
		def push_to_historical_collection_table(df_tweet_count, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity):
		...
		trucation(df_tweet_count, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity)
		push_to_historical_collection_table(df_tweet_count, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity)

	df_tweet_count, df_daily_tweetdist, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity, df_positive_profile, df_negative_profile= fetch_and_transform(uri,mydb)
	#pushdata(df_tweet_count, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity)

	return df_tweet_count, df_daily_tweetdist, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity, df_positive_profile, df_negative_profile
df_tweet_count, df_daily_tweetdist, df_tweet_dist, top_df, df_nps, df_reach, df_popular, df_more_retweet, df_entity, df_positive_profile, df_negative_profile  = TwitterAnalysis(uri,mydb)