def FacebookAnalysis(uri,mydb):
	...
	def fetch(uri,mydb):
		...
		df2_excel=pd.ExcelFile('FB_lookup_14Sep.xlsx',engine='openpyxl')
	...
	def transform(df1,df2):
		...
		df_reach=pd.DataFrame(columns=['date','reach'],data=[[last_date_of_week,df_reach]])
		df_summary=df_summary.head(5)
		df_comment_count=pd.DataFrame([[last_date_of_week,total_comments]],columns=["date","comments_count"])
		...
		return df, df_post_count, df_comment_count, df_user_count, df_reach, df_summary, top_df, df_overall_senti, df_most_active_perfile_and_nps, df_positive_posts, df_negative_posts,df_entity
	...
	df, df_post_count, df_comment_count, df_user_count, df_reach, df_summary, top_df, df_overall_senti, df_most_active_perfile_and_nps, df_positive_posts, df_negative_posts, df_entity = transform(df1, df2)
	...
	return df, df_post_count, df_comment_count, df_user_count, df_reach, df_summary, top_df, df_overall_senti, df_most_active_perfile_and_nps, df_positive_posts, df_negative_posts, df_entity
df, df_post_count, df_comment_count, df_user_count, df_reach, df_summary, top_df, df_overall_senti, df_most_active_perfile_and_nps, df_positive_posts, df_negative_posts, df_entity = FacebookAnalysis(uri,mydb)